...
def _run() -> None:
    ...
    # default) - see mock_flash()'s own comment. Left unset, this specific
    # assertion (a deterministic "does the happy path reach 100%" check,
    # not a test of that random branch) was a real, if rare, source of a
    # spurious failure with no code regression behind it. verify_can_ota.py
    # already avoids the exact same branch the same way for the exact same
    # reason - this just applies that established convention here too.
    panel2._allow_downgrade_check.setChecked(True)

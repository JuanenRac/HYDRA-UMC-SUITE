...
def _run() -> None:
    ...
    # own lifetime, never true for this script, which never enters a real
    # app.exec() loop). Left running, that armed QTimer plus this test's
    # own never-closed qasync.QEventLoop reliably crashed the interpreter
    # on exit (a real, deterministic native crash - STATUS_STACK_BUFFER_
    # OVERRUN on Windows - not a Python exception, and not visible at all
    # unless this exact script's own exit code is checked, which nothing
    # did until now). Every real assertion above already passed by this
    # point; this is real interpreter-shutdown hygiene, not test coverage.

...
- **Atomic per-command sync** (`POST /api/robot/:id/command`,
  `robot_control.py`) - jog (the joint knob/slider), speed/acceleration,
  and now play/pause/stop/tool/valve/pump all send this way instead of
  `push_state()`'s own full read-modify-write, matching what the Android
  app and HYDRA-UMC-STUDIO's own RobotDetail.tsx already do for these
  exact same controls. Play is disabled with nothing real in the
  robot's own server-synced `recordedPoints` to replay - the same
  `recordedPoints.length === 0` guard RobotDetail.tsx's own handlePlay()
  applies (this app's own `trajectory_panel.py` is a separate, local-only
  point list that never reaches that field - see its own header).
  "Valve"/"pump" here are the robot's own 2 generic ones

...
class RobotView:
    ...
    def is_playing(self) -> bool:
        return bool(self.playback_state.get("isPlaying", False))

    @property
    def is_paused(self) -> bool:
        return bool(self.playback_state.get("isPaused", False))

    @property
    def is_finished(self) -> bool:
        return bool(self.playback_state.get("isFinished", False))

    def start_playing(self) -> None:
        """Optimistic local mirror of server.ts's own 'play' case - see
        RobotDetail.tsx's handlePlay() for the same isPlaying/activeStep/
        isPaused/isFinished reset this mirrors."""
        pb = self.raw.setdefault("playbackState", {})
        pb.update(isPlaying=True, activeStep=0, isPaused=False, paused=False, isFinished=False, finished=False)

    def stop_playing(self) -> None:
        """Optimistic local mirror of server.ts's own 'stop' case."""
        pb = self.raw.setdefault("playbackState", {})
        pb.update(isPlaying=False, activeStep=-1, isPaused=False, paused=False, isFinished=False, finished=False)

    def set_paused(self, paused: bool) -> None:
        """Optimistic local mirror of server.ts's own 'pause' case - a
        real pause/continue only makes sense while already playing, same
        guard RobotDetail.tsx's own PAUSE/CONTINUE button applies
        (disabled unless `playbackState.isPlaying`)."""
        pb = self.raw.setdefault("playbackState", {})
        pb.update(isPlaying=True, playing=True, isPaused=paused, paused=paused, requestPause=paused, requestStop=False, isFinished=False, finished=False)

    @property
    def recorded_point_count(self) -> int:
        """Length of `recordedPoints` - HYDRA-UMC-STUDIO's own server-synced
        trajectory (RobotDetail.tsx's `robot.recordedPoints`), recorded from
        ANY client in the swarm, not the same thing as this app's own
        trajectory_panel.py (a separate, local-only point list that never
        reaches the server). Read-only here - this app doesn't record into
        it, only gates whether Play has anything real to replay."""
        points = self.raw.get("recordedPoints")
        return len(points) if isinstance(points, list) else 0

    @property
    def valves(self) -> list[bool]:
        """The robot's own 2 generic valves (RobotDetail.tsx's manual
        control panel, `selectedRobot.valves[0/1]`) - NOT the same field as
        a tool-attachment module's own `pumpActive`/`valveActive` (e.g.
        VacuumTable), which is a genuinely separate concept keyed by module
        instead of by robot (see vacuum_table_panel.py's own header)."""
        raw_valves = self.raw.get("valves")
        result = list(raw_valves) if isinstance(raw_valves, list) else []
        while len(result) < 2:
            result.append(False)
        return [bool(v) for v in result[:2]]

    def set_valve(self, index: int, state: bool) -> None:
        valves = self.valves
        valves[index] = state
        self.raw["valves"] = valves

    @property
    def pumps(self) -> list[bool]:
        """The robot's own 2 generic pumps - see `valves` above for the
        same real distinction from a module's own pump field."""
        raw_pumps = self.raw.get("pumps")
        result = list(raw_pumps) if isinstance(raw_pumps, list) else []
        while len(result) < 2:
            result.append(False)
        return [bool(v) for v in result[:2]]

    def set_pump(self, index: int, state: bool) -> None:
        pumps = self.pumps
        pumps[index] = state
        self.raw["pumps"] = pumps

...
- **🛠️ CNC / 激光工具附件配置** —— 启用/禁用、宽度/长度（毫米）尺寸、重置，由一个共享面板实现，移植自 HYDRA-UMC STUDIO 自身的 `CNC.tsx`/`Laser.tsx`（除模块键外完全相同的组件）。这是 STUDIO 已有的 11 个逐工具配置面板中的头 2 个；其余仍是一个真实、尚未解决的差距。
...
- 🎮 **数字孪生与仿真：** [HYDRA-UMC-TWIN](https://github.com/JuanenRac/HYDRA-UMC-TWIN) · [HYDRA-UMC-PHYSICS-REPLICA](https://github.com/JuanenRac/HYDRA-UMC-PHYSICS-REPLICA) · [HYDRA-UMC-SYNTHETIC-DATA-GEN](https://github.com/JuanenRac/HYDRA-UMC-SYNTHETIC-DATA-GEN)

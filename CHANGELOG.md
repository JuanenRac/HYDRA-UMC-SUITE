...
- **Atomic per-command sync extended to play/pause/stop/tool/valve/pump**
  (`robot_control.py`) - these now send the same atomic
  `POST /api/robot/:id/command` the joint knob and speed/acceleration
  sliders already used, instead of never being wired at all (this app
  had no UI for them before). Matches HYDRA-UMC-ANDROID-CONTROL's own
  atomic commands and HYDRA-UMC-STUDIO's own RobotDetail.tsx transport
  buttons/I/O tab exactly: Play is disabled with nothing real in the
  robot's own server-synced `recordedPoints` to replay (this app's own
  `trajectory_panel.py` is a separate, local-only point list that never
  reaches that field); "valve"/"pump" are the robot's own 2 generic ones
  (`RobotView.valves`/`.pumps`), a genuinely distinct field from a
  tool-attachment module's own `pumpActive`/`valveActive` (e.g.
  VacuumTablePanel's), which still writes via `push_active_state()` and
  is unrelated to this change. New `tests/verify_robot_control_panel.py`
...
- **REV-019 (found in an independent revalidation audit, P2): neither
  ...
  that stopped reflecting real state, or a panel that crashed on
  construction could reach `main` with a fully green CI. New
  `tools/run_offline_verifiers.py` discovers and runs every
  `tests/verify_*.py` script under a headless Qt platform
  (`QT_QPA_PLATFORM=offscreen`), deliberately never
  `tests/test_net_manual.py` (needs a real, already-running
  HYDRA-UMC-SERVER, excluded by the glob itself). Wired into both
  `tools/build_test.py` (local, `is_file()`-gated so it stays a no-op
  for any other repo reusing that same generic script) and this
  repository's own CI workflow (new `pip install -r requirements.txt` +
  a small `apt-get` step for PySide6's offscreen platform runtime
  libraries, both gated on the runner script's own existence).
  **Two real, pre-existing bugs surfaced immediately by actually running
  all 21 scripts back to back for the first time, both fixed here too:**
  `verify_cameras_panel.py` reliably crashed the interpreter on exit
  (`STATUS_STACK_BUFFER_OVERRUN` on Windows) because `CamerasPanel`
  starts a real, unconditional 3s `QTimer` at construction that this
  script never stopped, alongside its own never-closed
  `qasync.QEventLoop` - both are now cleaned up explicitly before the
  script exits. `verify_flasher_panel.py` had a real, if rare
  (empirically ~1 in 5-10 runs), source of a spurious failure: its own
  end-to-end mock-flash assertion didn't set `allow_downgrade=True`
  first, leaving `can_ota.py`'s own deliberate ~3% unseeded
  anti-rollback simulation free to trigger - `verify_can_ota.py` already
  avoided that same branch the same way; this just applies that
  existing convention here too. Sanity-checked live: a deliberately
  broken real assertion (a wrong reset-default constant) was confirmed
  to fail both the individual script and the batch runner, then
  reverted. All 21 scripts now pass reliably, repeatedly, in a real
  local run of `tools/build_test.py`.

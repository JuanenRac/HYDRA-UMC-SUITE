...
**Honesty check - what actually runs today:** network discovery (concurrent subnet scan + real mDNS/Bonjour, `hydra_suite/net/discovery.py`), the live REST+WebSocket connection and swarm support (`hydra_suite/net/client.py`, `hydra_suite/app.py`), the 3D viewport's real forward kinematics for all 24 real robot models plus a primitive "Generic" fallback (`hydra_suite/render/`, numerically verified bit-for-bit against HYDRA-UMC STUDIO's own TypeScript FK), all 11 of 11 tool-attachment config panels, CAN-OTA/SPI-OTA flashing, and real MJPEG camera rendering are real and tested - 28 offline verify/smoke scripts under `tests/` (`python tools/run_offline_verifiers.py`, headless Qt via `QT_QPA_PLATFORM=offscreen`), all passing, plus `tests/smoke_test_app.py`/`tests/smoke_test_viewport.py` exercised against a real running HYDRA-UMC SERVER and a real GPU/OpenGL context on this machine. The Qt Quick shell (`--qtquick`, all 26 classic panels ported, including the 3D viewport through a dedicated `OffscreenRobotRenderer`) is real, working code, launched alongside the unchanged classic entry point. What is explicitly NOT done yet, per this repo's own honesty convention: the URTC Tool Head/Advanced Expansion CAN-OTA tiers have no real hardware relay tunnel; and `combinedWith` (combined-robot mode) isn't exposed in this app's UI despite existing in STUDIO's own data model. `trajectory_panel.py` now exports/imports through HYDRA-UMC STUDIO's own real `WORKS/*.json` format (`POST /api/upload-work`, `GET /<folder>/index.json`) - a point recorded here is a real, playable Work from either app. See [`docs/ROADMAP.md`](docs/ROADMAP.md) for the complete, itemized real-vs-deferred breakdown this paragraph summarizes, and `CHANGELOG.md` for exactly what has shipped so far.
...
### Full Qt Quick redesign (`--qtquick`)

That command deck is a layer over the classic `QMainWindow`. This app also has a completely separate, standalone Qt Quick shell:

~~~
python main.py --qtquick
~~~

A genuine QML `ApplicationWindow` - not embedded in the classic window at all, unlike the command deck above (both real ways of embedding QML inside the classic `QMainWindow`/`QDockWidget` tree were tried and abandoned: `QQuickWidget` painted solid black, and `QQuickView`+`createWindowContainer()` rendered correctly in isolation but corrupted sibling docks' real Z-order once placed inside this app's actual 26-dock layout). The same real, proven pattern HYDRA-UMC-OS-REBUILDER, HYDRA-UMC-UPDATER, URTC-TESTER, URTC-FLASHER and HYDRA-UMC-EDITOR-URDF already use, launched alongside the unchanged classic entry point rather than replacing it. Trades `QDockWidget`'s float/split/tab-merge flexibility for STUDIO's own simpler nav-sidebar-plus-single-content-pane shape (mirroring `nav_sidebar.py`'s own real taxonomy item for item) - all 26 real classic panels are ported to real QML content, including the 3D Viewport, whose live preview is fed through a dedicated `OffscreenRobotRenderer` (a genuine separate `QOpenGLContext`/`QOffscreenSurface`/framebuffer, deliberately not Qt Quick's own `QQuickFramebufferObject`, which would force the whole app's Quick backend off Windows' real default Direct3D11 onto OpenGL) reusing the exact same real rendering code (`RobotGLRenderer`) the classic viewport widget uses, via a `QQuickImageProvider`.
...
- **🛠️ Tool-attachment config, all 11 of 11 panels** - CNC, Laser,
  ...
  needed their own real, purpose-built panel. Heated beds and vacuum tables use STL in `render/module_rig.py`. PnP, CNC and laser use `render/pnp_rig.py`: 7 articulated meshes and 160 individual parts per independent machine directory. `RobotViewport` renders each assembly. See [machine assets](docs/MACHINE_ASSETS.md).
...
├── verify_qt_suite_shell.py        # Real, hardware/network-free Qt Quick shell check - deliberately kept outside tests/ (same reason as URTC-TESTER/URTC-FLASHER's own verify_qt_*.py), run with `QT_QPA_PLATFORM=offscreen python verify_qt_suite_shell.py`
├── requirements.txt
├── hydra-umc.project.json         # Ecosystem manifest - version/family/parent, the source dashboard/updater/OS-REBUILDER read
├── bump_version.py                # Odometer version bump for hydra_suite/__init__.py's own __version__, run by build_exe.bat/.sh before every real PyInstaller build
├── bump_manifest_version.py       # Syncs hydra-umc.project.json's version to the native one (generic, copied verbatim across the ecosystem)
├── build.bat / build.sh           # venv + editable install + real test suite (incremental, versioning build)
├── build-test.bat / build-test.sh # Same checks, non-mutating - never bumps the version or touches CHANGELOG.md
├── run.bat / run.sh               # Launches main.py through the venv
...
├── README.md                      # this file
├── README_spa.md / README_fra.md / README_ita.md / README_deu.md / README_zho.md / README_jpn.md  <- translations
├── .github/                        # CI workflow, issue templates, PR template (generic, ecosystem-shared)
├── hydra_suite/
│   ├── models.py                   # HydraState/ControllerView/RobotView/CameraView - thin, mutation-friendly views over the real settings.json shape
...
│   ├── can_ota.py                  # Shared CAN-OTA/SPI-OTA transport (real port of STUDIO's own canOta.ts) - used by Flasher and Tester
│   ├── logging_handler.py          # Routes Python logging into the Logs panel
│   ├── net/
│   │   ├── discovery.py             # Concurrent subnet scan + real mDNS (_hydra._tcp) against GET /api/hydra-info, deduplicated
│   │   └── client.py                # Per-server REST + WebSocket connection, live bidirectional sync, login, admin/discovery/PTZ endpoints
│   ├── render/
...
│   │   ├── module_rig.py            # Tool-attachment module geometry (CNC/Laser/Heated Bed/Vacuum Table)
│   │   ├── pnp_rig.py               # Real Cartesian-gantry chain for the LumenPnP/JuanenPnP mesh rig
│   │   ├── mesh.py                  # STL loading (numpy-stl)
│   │   └── viewport.py              # RobotGLRenderer (real GLSL shader pipeline, orbit camera) + the classic QOpenGLWidget wrapper + OffscreenRobotRenderer for the `--qtquick` deck's own 3D Viewport panel
│   └── ui/
│       ├── main_window.py           # QMainWindow + QDockWidget workspace
│       ├── about_dialog.py          # Real About dialog (version/author/license)
...
│       └── panels/                   # One file per dock panel - real 1:1 parity with STUDIO's own tabs: server_browser, overview, robot_control, viewport_panel, trajectory_panel, cameras_panel (+ real PTZ control), ai_family_status_panel, ecosystem_services_panel, ecosystem_telemetry_panel, admin_clients_panel, admin_logs_panel, admin_server_panel, system_supervisor_panel, logs_panel, module_config_panel (+cnc/laser/heated_bed/vacuum_table), atc_tools_panel, xy_table_panel, rack_config_panel, pick_and_place_panel, kinematic_brain_stage_panel, flasher_panel, tester_panel
├── assets/
│   ├── qss/industrial_dark.qss     # The futuristic-industrial Qt stylesheet
│   ├── qml/Main.qml                 # Qt Quick UI for the `--qtquick` command deck (all 26 panels)
│   └── meshes/                      # Real STL meshes, one folder per robot/module, copied from HYDRA-UMC-STUDIO's own public/models/ (each with its own ATTRIBUTION.txt)
├── language/                        # english/spanish/french/german/italian/japanese/chinese .lng files
├── docs/
│   └── ROADMAP.md                   # Honest real-vs-not-yet scope statement
├── tools/
│   ├── build_test.py                # Non-versioning build/compile check (generic, ecosystem-shared) - also runs run_offline_verifiers.py below when present
│   ├── ci_validate.py               # Manifest/CHANGELOG/docs validation used by CI (generic, ecosystem-shared)
│   └── run_offline_verifiers.py     # `python tools/run_offline_verifiers.py` - every tests/verify_*.py under a headless Qt platform (QT_QPA_PLATFORM=offscreen), real regression coverage for each panel's own Qt/backend wiring
├── tests/                           # Real headless test suite (QApplication, no display needed) - one verify_*.py per panel/subsystem, plus kinematics ports and manual smoke tests that need a real running STUDIO server
├── installer/                       # Platform packaging notes/assets
...
- **[docs/ROADMAP.md](docs/ROADMAP.md)** — what's real and verified end-to-end today versus what's deliberately still out of scope.
- **[installer/README.md](installer/README.md)** — building the Windows `.exe` installer and the Linux `.deb` package.
- **[CONTRIBUTING.md](CONTRIBUTING.md)** — tech stack and coding guidelines for a pull request.
- **[CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md)** — the standards of behavior expected in this community.
- **[SECURITY.md](SECURITY.md)** — how to report a vulnerability, and this app's own real security focus areas.
- **[SUPPORT.md](SUPPORT.md)** — where to ask questions and report bugs.
- **[LICENSE.md](LICENSE.md)** — this project's own license, plus the separate license each `assets/meshes/` folder is redistributed under.
